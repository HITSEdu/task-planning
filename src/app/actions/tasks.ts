...
export async function assignUserAction(
  ...
  const result = await Promise.all(
    users.map((u) => dal.assignUser(taskId, { UserId: u })),
  );
  if (result.every((r) => r.status === "success")) {
    ...
    return {
      status: "success",
      message: "Выбранные пользователи успешно назначены",
    } as StateType;
  } else {
    revalidatePath(
      "/teams/[teamId]/projects/[projectId]/tasks/[taskId]",
...
export async function selfAssignAction(
  taskId: string,
  _prev: StateType,
): Promise<StateType> {
  const dal = await TaskDAL.create();
...
export async function updateTaskAction(
  ...
  const result = await dal.updateTask(taskId, {
    title: formData.get("title"),
