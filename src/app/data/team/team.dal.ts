...
export class TeamDAL {
  ...
  async getUserTeams() {
    ...
    const activeTeams = teamMapper(
      userTeams.filter((ut) => ut.role !== "PENDING"),
    );

    const pendingInvites = teamMapper(
      userTeams.filter((ut) => ut.role === "PENDING"),
    );
  ...
  async addUserToTeam(
    ...
    if (mates.map((m) => m.user.username).includes(parsed.data.username)) {
      return {
        status: "error",
        message:
          "Пользователь уже состоит в команде или ожидает подтверждения!",
      };
    }
