...
export default function TaskItem({ task, role, assignedUsers }: TaskItemProps) {
  return (
    <Item variant="default" className="flex flex-col gap-6 p-4 items-start">
      <ItemContent className="w-full">
        ...
        <ItemDescription>Описание: {task.description ?? "-"}</ItemDescription>
        ...
        <div className="mt-4 w-full">
          ...
          <div className="flex flex-col gap-2">
            {assignedUsers.map((user) => (
              <Card key={user.id} className="bg-muted/40">
