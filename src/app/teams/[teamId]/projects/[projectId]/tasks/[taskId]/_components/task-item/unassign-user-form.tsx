...
export default function UnassignUserForm({
  ...
  const [state, action, pending] = useActionState(unassignUserAction.bind(null, task.id, userId), {})
  ...
  return (
    <Dialog>
      <Button
        asChild
      >
        <DialogTrigger>{pending ? 'Снятие...' : 'Снять назначение'}</DialogTrigger>
      </Button>
      <DialogContent>
        <DialogHeader>
          ...
          >
            <DialogClose asChild>
              <Button
                type="submit"
                variant="secondary"
              >{pending ? 'Снятие...' : 'Снять назначение'}</Button>
            </DialogClose>
          </form>
          <DialogClose asChild>
