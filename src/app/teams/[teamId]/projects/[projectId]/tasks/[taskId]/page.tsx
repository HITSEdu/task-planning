...
export default async function TaskPage({ params }: Props) {
  ...
  const unAssignedUsers = teamMembers.filter(
    (u) => !assignedUsers.map((i) => i.id).includes(u.id),
  );
      ...
      <section className="flex-1 flex flex-col">
        <Card className="h-full flex flex-col bg-card/70 backdrop-blur shadow-md">
          ...
          <CardContent className="flex flex-col gap-4 mt-4 items-start">
            {isAssigned ? (
              <UnassignUserForm task={task} userId={user.id} />
            ) : (
            ...
            {isOwner && (
              <>
                ...
                <EditTaskForm task={task} availableTasks={allTasks} />
                <AssignUserForm task={task} availableUsers={unAssignedUsers} />
              </>
            )}
          </CardContent>
        </Card>
